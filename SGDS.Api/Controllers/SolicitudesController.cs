...
// PUT: api/Solicitudes/5/asignar-usuario
    ...
    // La lógica de negocio pura vive en SGDS.Application.Helpers.ReglasComfenalco (probable con
    // xUnit sin depender de EF/HTTP) — aquí solo queda la resolución del TipoSolicitud por id.
    ...
    private async Task<string?> ValidarReglasComfenalcoAsync(int? tipoSolicitudId, string? datosAdicionalesJson)
    {
        ...
        return ReglasComfenalco.Validar(tipo.Nombre, LeerDatosAdicionales(datosAdicionalesJson));
    }
    ...
    private async Task<(IActionResult? Error, Solicitud? Solicitud, CarneVirtualResponseDto? Dto)> CalcularCarneVirtualAsync(int id)
    {
        ...
        var categoria = ReglasComfenalco.CalcularCategoriaCarne(ingresos);
            ...
            try
            {
                ...
                foreach (var b in doc.RootElement.EnumerateArray())
                {
                    ...
                    grupoFamiliar.Add(new BeneficiarioCarneDto
                    {
                        ...
                        Categoria = ReglasComfenalco.CalcularCategoriaCarne(ingresosBeneficiario),
                    });
                }
            }
            catch (JsonException) { }
